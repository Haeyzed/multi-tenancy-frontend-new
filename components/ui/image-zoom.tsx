...
// `react-medium-image-zoom` references the DOM `Element` interface at module
// top level, which crashes Next.js during static prerender / build. Loading
// the runtime via `next/dynamic` with `ssr: false` defers the require until
// the client hydrates, so the build can statically prerender the page shell
// without ever evaluating browser-only code.
const Zoom = dynamic(() => import("react-medium-image-zoom").then(m => m.default), {
  ssr: false,
}) as unknown as ComponentType<UncontrolledProps & Partial<ControlledProps>>
