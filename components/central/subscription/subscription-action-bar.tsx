...
export function SubscriptionActionBar({ table }: SubscriptionActionBarProps) {
  const queryClient = useQueryClient()
  const { can } = usePermissions()
  const [deleteOpen, setDeleteOpen] = React.useState(false)

  const rows = table.getFilteredSelectedRowModel().rows
  const canDelete = can(Permissions.billing.manage)
        ...
        <DeleteConfirmDialog
          ...
          description={
            <>
              ...
              selected subscription{rows.length === 1 ? "" : "s"}. This action
              cannot be undone.
            </>
          }
