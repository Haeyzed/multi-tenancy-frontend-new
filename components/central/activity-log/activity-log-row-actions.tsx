"use client"

import { EyeIcon, MoreHorizontalIcon } from "lucide-react"
import * as React from "react"

import { RecordViewDialog } from "@/components/central/record-view-dialog"
import { Button } from "@/components/ui/button"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu"
import { usePermissions } from "@/hooks/use-permissions"
import { Permissions } from "@/lib/central/auth/permissions"
import { getActivityLogViewFields } from "@/lib/central/view/view-fields"
import type { ActivityLogEntry } from "@/types/central/activity-log"

interface ActivityLogRowActionsProps {
  entry: ActivityLogEntry
}

export function ActivityLogRowActions({ entry }: ActivityLogRowActionsProps) {
  const { can } = usePermissions()
  const [viewOpen, setViewOpen] = React.useState(false)

  const canView = can(Permissions.platform.view)

  if (!canView) {
    return null
  }

  return (
    <>
      <DropdownMenu>
        <DropdownMenuTrigger
          render={
            <Button variant="ghost" size="icon">
              <MoreHorizontalIcon className="size-4" />
              <span className="sr-only">Open menu</span>
            </Button>
          }
        />
        <DropdownMenuContent align="end">
          <DropdownMenuItem onClick={() => setViewOpen(true)}>
            <EyeIcon />
            View details
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>

      <RecordViewDialog
        open={viewOpen}
        onOpenChange={setViewOpen}
        title={entry.description}
        description="Audit activity entry"
        fields={getActivityLogViewFields(entry)}
      />
    </>
  )
}
